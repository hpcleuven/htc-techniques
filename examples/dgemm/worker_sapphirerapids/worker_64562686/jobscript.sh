...
#SBATCH --time=1:00:00
#SBATCH --job-name=worker_08_tasks
#SBATCH --nodes=1
#SBATCH --ntasks=8
...
#   * server_info (/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/server_info.txt)
#   * server_log_opt, e.g., --log path_to_log_file (--log "/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/server.log")
#   * port_opt, e.g., --port 1234 (--port 5555)
#   * server_start_delay, 5 (in seconds) (5)
#   * workfile (/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/workerfile.txt)
#   * client_log_prefix_opt, e.g., --log_prefix client_log_prefix (--log_prefix "/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/client_")
...
srun --exclusive --het-group=1 --nodes=1 --ntasks=1 --cpus-per-task=1 \
    ...
    "${worker_server_exec}" \
        --log "/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/server.log" \
        --port 5555 \
        --workfile "/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/workerfile.txt" \
...
do
    srun --exclusive --het-group=0 \
        ...
        --threads-per-core=1 \
            "${worker_client_exec}" \
                --server "$server" --uuid "$uuid" --log_prefix "/vsc-hard-mounts/leuven-data/307/vsc30745/repos/github/hpcleuven/htc-techniques/examples/dgemm/worker_${SLURM_JOB_ID}/client_" \
